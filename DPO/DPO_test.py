...
from tinker_cookbook.recipes.preference.datasets import (
    ...
    LabeledComparison,
    hhh_example_to_comparison
)
from tinker_cookbook.preference.dpo_datasets import ComparisonDatasetBuilder
...
# DPO_test.py (or custom_dpo_dataset.py)
import json


@chz.chz
class customComparisonBuilder(ComparisonDatasetBuilder):
    ...
    def get_train_and_test_datasets(self) -> tuple[datasets.Dataset, datasets.Dataset | None]:
        dataset = datasets.load_dataset("json", data_files={"train": self.path})   # doing test with anthropic since I know this should work
...
class CLIConfig:
    ...
    dataset: str = "DPO\example_dpo_data.json"  # https://huggingface.co/datasets/HuggingFaceH4/hhh_alignment
...
def get_dataset_builder(
    ...
    if dataset == "hhh":
        ...
        return DPODatasetBuilderFromComparisons(
            ...
            comparison_builder=customComparisonBuilder(path=dataset),)
